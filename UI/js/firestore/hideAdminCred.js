...
const authClientLinks = document.querySelectorAll(".clients-login");
const pClientLinks = document.querySelectorAll(".pclient-links");
const pAuthClientLinks = document.querySelectorAll(".plogged-in");
...
auth.onAuthStateChanged((user) => {
  if (user) {
    ...
    if (providerHolder === "password") {
      ...
      authClientLinks.forEach((link) => {
        link.style.display = "flex";
      });
      pAuthClientLinks.forEach((link) => {
        link.style.display = "block";
      });
    }
  } else {
